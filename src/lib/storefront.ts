...
export function buildProductDetail(p: ApiProduct): ProductDetailData {
  ...
  const sizing = m.sizing && Object.values(m.sizing).some(Boolean) ? m.sizing : undefined
...
export async function fetchRecommendations(limit = 8): Promise<RecommendationCardData[]> {
  const products = await api.get<ApiProduct[]>(`/products/featured?limit=${limit}`)
...
export async function fetchFeaturedReviews(limit = 6): Promise<FeaturedReviewApi[]> {
...
export async function fetchProductReviews(productId: string): Promise<ProductReviewApi[]> {
 ...
 */
export async function submitProductReview(review: NewProductReview): Promise<void> {

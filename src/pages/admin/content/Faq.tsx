    ...
    <div className="p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{items.length} question(s)</p>
        </div>
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-border">
            {items.map((it) => (
              <div key={it.id} className="flex items-start gap-4 px-6 py-4 hover:bg-muted/40 transition-colors">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-foreground">{it.question}</span>
                    {!it.isActive && (
                      <span className="px-2 py-0.5 rounded-full text-xs bg-muted text-muted-foreground">Inactif</span>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{it.answer}</p>
                </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-card rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">Réponse *</label>

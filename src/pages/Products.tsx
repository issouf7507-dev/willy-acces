...
import { BAGS, SORT_OPTIONS, type BagProduct } from '../data/bags'
...
function buildCategories(): Category[] {
  const defs = [
    ...
    { id: 'velo',      label: 'Vélo',           filter: (p: BagProduct) => p.tags.includes('bike bag') },
    { id: 'pochettes', label: 'Pochettes',      filter: (p: BagProduct) => p.name.toLowerCase().includes('pouch') || p.name.toLowerCase().includes('flap') },
  ]
  return defs.map(d => ({ ...d, count: BAGS.filter(d.filter).length }))
}
...
function BestSellersStrip() {
  const top = useMemo(() => [...BAGS].sort((a, b) => b.reviews - a.reviews).slice(0, 4), [])
  return (
...
export default function Products() {
  ...
  const filtered = useMemo(() => {
    let result = BAGS.filter(activeCat.filter)
    ...
    return result
  }, [activeCategory, sortBy, search, activeCat])
      ...
      <section className="bg-black text-white overflow-hidden relative">
        <div className="max-w-[1600px] mx-auto px-5 md:px-12 py-16 md:py-24">
          ...
          <p className="text-sm text-zinc-400 max-w-sm mt-6">
            {BAGS.length} articles — sacs, slings, messagers et accessoires pour le quotidien.
          </p>
        </div>
          ...
          {CATEGORIES.map(cat => (
            ...
            >
              ...
              <span className={`ml-1.5 text-[10px] ${activeCategory === cat.id ? 'text-zinc-500' : 'text-zinc-300'}`}>
                {cat.id === 'all' ? BAGS.length : cat.count}
              </span>
            </button>
          ))}
        </div>
